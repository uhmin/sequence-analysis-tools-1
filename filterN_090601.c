  ...
  goto blast_start;
	...
	*/
	...
	preAlignmentScore=score;
	     ...
	     /*Alignment_count!=Alignment_number &&*/
	     alignment_status!=1);
...
char *search_header(void){
  while(fileend!=NULL){
    /*  printf("%s",global_tmp);*/
    if (strncmp(global_tmp,"BLAST", 5)==0 
	|| strncmp(global_tmp,"TBLAST", 6)==0) {
      break;
    }
    fileend=fgets_wrap(0, global_tmp,MAXLETTER-1,fin);
  }
...
long search_query_length(void){
  ...
  while(strstr(global_tmp,"letters)")==NULL){
...
char *tinypiece(void){
  while(1){
    ...
    /*    if (cp1!=NULL) break;*/
    if(strncmp(global_tmp,"Query:", 5)==0) break;
    if (strncmp(global_tmp,"BLAST", 5)==0 ||
	strncmp(global_tmp,"TBLAST", 6)==0) {
